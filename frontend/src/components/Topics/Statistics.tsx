...
import ms from "ms";
import { ChartNoAxesCombined, Clock, FileText, Mail } from "lucide-react";
...
function Statics() {
    ...
    return <div className="h-fit lg:h-35 py-5 px-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">{
        statistics
            ? <>
                {[
                    { label: 'Topics', value: `${statistics.total_topic}`, icon: FileText, hiddenOnMobile: true },
                    { label: 'Submissions', value: statistics.total_submission, icon: Mail },
